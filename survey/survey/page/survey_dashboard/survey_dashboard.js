frappe.pages["survey_dashboard"].on_page_load = function (wrapper) {
	...
	$container.append(
		'<div id="survey-container" style="height: calc(100vh - 150px); overflow: auto;"></div>'
	);
...
function render_builder_v2($container, survey_id) {
	...
	if (
		...
		window.SurveyCreatorCore.registerSurveyTheme &&
		window.SurveyTheme
	) {
	...
	if (component) {
		...
		$container.html(
			'<div class="alert alert-danger">SurveyJS Creator library failed to initialize correctly. Please check console for "ComponentFactory" error.</div>'
		);
	}
		...
		callback: function (r) {
			if (r.message && r.message.survey_json) {
				...
				const unlock = () => {
					...
					if (creator.propertyGrid && creator.propertyGrid.survey) {
						...
						pgSurvey.getAllQuestions().forEach((q) => {
							...
							q.enabled = true;
							try {
								q.setPropertyValue("readOnly", false);
							} catch (e) {
								// setPropertyValue failed, ignore
							}
						});
					}
				...
				if (!initialTheme && allThemes.length > 0) {
					initialTheme =
						allThemes.find((t) => t.themeName === "defaultV2") ||
						allThemes.find((t) => t.themeName === "default") ||
						allThemes[0];
				}
	...
	creator.saveSurveyFunc = (saveNo, callback) => {
		...
		frappe.call({
			...
			callback: (r) => {
				...
				if (!r.exc) {
					frappe.show_alert({
						message: __("Saved Survey and Theme"),
						indicator: "green",
					});
				}
			},
...
function render_analysis($container, survey_id) {
	...
	if (!window.SurveyAnalytics) {
		$container.html(
			'<div class="alert alert-danger">SurveyJS Analytics library not loaded.</div>'
		);
		return;
	}
			...
			frappe.call({
				...
				callback: function (data_res) {
					...
					if (!results || results.length === 0) {
						$container
							.find("#summaryContainer")
							.html(
								'<div class="text-center p-5">No responses found for this survey yet.</div>'
							);
						return;
					}
					...
					try {
						...
						console.log(
							"Analysis: Render called. Visualizers:",
							vizPanel.visualizers.length
						);
					} catch (e) {
					...
					const cleanBanners = () => {
						const banners = document.querySelectorAll(
							".sa-commercial-license-banner, .sa-view-container__banner, .sa-visualization-panel__banner"
						);
						...
						allDivs.forEach((div) => {
							if (
								div.innerText &&
								(div.innerText.includes("license is required") ||
									div.innerText.includes("To use the Dashboard library"))
							) {
								if (div.children.length < 3) {
									// Highly likely a banner, not a chart
